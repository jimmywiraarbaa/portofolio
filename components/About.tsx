...
interface Tab {
  ...
  gpa?: string;
  details?: string[];
  sliderItems?: { title: string; issuer: string; year: string }[];
  image: string;
  message?: string;
}
    ...
    <section id="education" className="py-32 md:py-48 bg-[var(--background)]">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <AnimatePresence mode="wait">
          {currentTab.message ? (
            // Message layout (for non-formal, courses)
            <motion.div
              key={activeTab}
            ...
            >
              ...
              <motion.p
                ...
                className="text-2xl md:text-4xl text-[var(--foreground)] text-center font-medium"
              >
                {currentTab.message}
              </motion.p>
            </motion.div>
          ) : (
            // Default grid layout (for formal)
            <motion.div
              key={activeTab}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
